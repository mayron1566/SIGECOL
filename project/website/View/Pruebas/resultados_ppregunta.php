...
//se guarda el id de la primera pregunta y se incrementa aux hasta que se vuelva a repetir el id de la primera pregunta7
    ...
    echo '<thead><tr><th>Alumno</th><th>Colegio</th><th>Rut</th><th>Fecha</th>';
    $k=0;
...
//guarda datos en matriz
	...
	if ($i==0){
		...
		echo '<tr id="',$resultados['nombre'],'">',"\n"; 
        	echo '<td>',$nom,'</td>',"\n";
		echo '<td>',$ape,'</td>',"\n";
		if(preg_match('!\d+!', $ruttemp)){
			preg_match_all('!\d+!', $ruttemp, $rut);
			$rut = implode('',$rut[0]);
	...
	for ($x=0; $x<$aux3; $x++){
		...
		echo '<th>'.number_format ( $media[$k]/$j , $decimals = 4).'</th>';
		$k++;		
		}
